...
/**
 * Hang up an outbound call (the given call object, else the live one) —
 * the single hangup path for the dialer, deal-page CommBar and CallDock.
 *
 * NOTE (9/21): a 7s "short-duration floor" briefly lived here (Telnyx
 * surcharges accounts where >15% of answered calls are ≤6s). Deferring the BYE
 * made the call's hangup event land after the rep had already dispositioned,
 * re-opening the disposition flow — Kyle pulled it and is training reps to
 * stay on the line past 6s instead. Hang up immediately.
 */
export function hangupOutbound(call?: any): void {
