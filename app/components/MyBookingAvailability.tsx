...
/**
 * A guide's own booking availability — weekdays, hours (Pacific) and days off
 * — layered over the team defaults. Used on My Profile (own) and, with repId,
 * inside the admin Booking settings for any guide.
 */
...
export function MyBookingAvailability({ repId, compact }: { repId?: string; compact?: boolean }) {
  ...
  const save = async () => {
    ...
    const r = await fetch(`/api/settings/booking-availability${qs}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ hours }) });
  ...
  return (
    <div className="card" style={{ marginBottom: 16, ...(compact ? { padding: 14 } : {}) }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <button className="btn primary" style={{ padding: "6px 16px", fontSize: 13.5 }} disabled={saving} onClick={save}>{saving ? "Saving…" : "Save availability"}</button>
