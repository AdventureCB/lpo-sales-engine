...
import { availableSlots, bookableReps, loadBookingConfig } from "@/lib/booking";
 ...
 * round-robin pool (?rep=rr). The customer's browser renders them in the zone
 * they choose; reps' hours are Pacific.
 */
export async function GET(req: NextRequest) {
  ...
  if (slug === "rr") {
    const slots = await availableSlots(db, { reps }, cfg);
