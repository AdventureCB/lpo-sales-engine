-- The authoritative "already bought" record is sales_orders (real Shopify
-- orders, 800+), NOT the Klaviyo order metrics — those only exist for events
-- ingested going-forward, so historical customers (e.g. cdavis, 2 orders)
-- slipped through and got recovery deals. Exclude anyone with a Shopify order,
-- plus (belt + suspenders) the order signals and open/won deals.
