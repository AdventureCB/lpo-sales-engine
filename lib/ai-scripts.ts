...
const CALL_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      plan: { type: "array", items: { type: "string" }, description: "The simple plan as clear next steps — ideally LPO's three-step form: (1) something I'll do, (2) something you should do, (3) something we'll do together, naming a specific day for the 'together' step. 3-4 short beats." },
      ...
      cta: { type: "string", description: "ONE clear ask sized to the buyer's stage — NOT always a purchase. Pick the right rung: join the community app (Trailhead), 'can I send info — email or text?', set up a demo, build in the 3D builder, apply for financing, add an accessory, or place a deposit. Early-stage → permission/community, not a deposit." },
...
export async function generateCallScript(
  ...
  const call = await callClaudeTool({
    ...
    systemCached: [
      ...
      `These outlines are scored by the same rubric that reviews the calls, so make the two structural moves unmistakable: (1) PLAN = LPO's three-step next steps — something I'll do, something you should do, something we'll do together, with a named day; (2) CTA = one clear ask sized to the buyer's stage (community app / send info / demo / 3D build / financing / accessory / deposit), never a deposit ask on an early-stage prospect.`,
